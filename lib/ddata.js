...
function link(id, options){
    options.hash = { id: id };
    var linked = identifier(options);
    var outputContext = {};
    if (linked){
        outputContext.name = linked.name;
        outputContext.url = isExternal.call(linked) ? linked.see : "#" + anchorName.call(linked);
    } else {
        outputContext = { name: id, url: null };
    }
    return options.fn(outputContext);
}
...
function parseLink(text) {
    ...
    var result = [];
    var matches = null;
    var link1 = /{@link (.*?)}/g;             // {@link someSymbol}
    var link2 = /\[(.*?)\]{@link (.*?)}/g;    // [caption here]{@link someSymbol}
    var link3 = /{@link (.*?)\|(.*?)}/g;      // {@link someSymbol|caption here}
