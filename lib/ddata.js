...
@module
@todo remove coupling with handlebars (should not assume data is consumed only by handlebars).. only
want functions to query common-doc-format / doc-data 
@example
```js
var handlebars = require("handlebars")
...
//         console.log(namepath, linked.kind)
...
function _identifiers (options) {
  ...
  return options.data.root.where(query).val()
}
