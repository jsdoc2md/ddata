...
exports.setAddSourceLinks = setAddSourceLinks;
exports.setBaseUrl = setBaseUrl;

var ADD_SOURCE_LINKS;
var BASE_URL = '';

function setAddSourceLinks(value) {
    ADD_SOURCE_LINKS = value;
}
...
function children(options){
    // return handlebars.helpers.each(_children.call(this, options), options);
...
function sig(options){
    ...
    if(ADD_SOURCE_LINKS && data.root) {
        var doclets = data.root.filter(function (item) {
            return item.name === data.name;
        });
        var doclet = doclets[0];

        if(doclet && doclet.meta) {
            data.meta = doclet.meta;

            var filePath = path.relative(process.cwd(), path.join(data.meta.path, data.meta.filename));
            var sourceUrlObj = url.parse(url.resolve(BASE_URL, filePath));
            sourceUrlObj.hash = '#L' + data.meta.lineno;

            data.meta.href = url.format(sourceUrlObj);
        }
    }
...
function exported(options){
    var exp = a.findWhere(options.data.root, { "!kind": "module", id: this.id });
    return exp || this;
}
