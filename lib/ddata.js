...
function identifiers(options){
    return boil._handlebars.helpers.each(identifiers(options), options);
}
...
function orphans(options){
    return boil._handlebars.helpers.each(orphans(options), options);
}
...
function module(options){
    options.hash.kind = "module";
    var result = identifiers(options)[0];
...
function showMainIndex(options){
    return orphans.call(this, options).length > 1;
}

/**
Returns an array of the top-level elements which have no parents
@returns {array}
@alias module:ddata.ophans
*/
function orphans(options){
