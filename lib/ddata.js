...
function sig(options){
    var output = {
        prefix: this.kind === "constructor" ? "new" : "",
        parent: null,
        accessSymbol: null,
        name: isEvent.call(this) ? '"' + this.name + '"' : this.name,
        methodSign: null,
        returnSymbol: null,
        returnTypes: null,
        suffix: this.isExported ? "⏏" : isPrivate.call(this) ? "℗" : "",
        depOpen: null,
        depClose: null,
        codeOpen: null,
        codeClose: null,
    ...
    if (isConstructor.call(this) || isFunction.call(this)){
        output.methodSign = "(" + mSig + ")";
    } else if (isEvent.call(this)){
        if (mSig) output.methodSign = "(" + mSig + ")";
    }
    
    if (!isEvent.call(this)){
        output.parent = parentName.call(this, options);
        output.accessSymbol = (this.scope === "static" || this.scope === "instance") ? "." : this.scope === "inner" ? "~" : "";
    }
    ...
    if (!isConstructor.call(this)){
        if (this.returns){
            ...
            if (typeNames.length){
                output.returnTypes = a.flatten(typeNames);
            }
        } else if ((this.type || this.kind === "namespace") && this.kind !== "event"){
            output.returnSymbol = ":";
            
			if (this.kind === "namespace"){
	            output.returnTypes = [ "object" ];
			} else {
	            output.returnTypes = this.type.names;
			}
        } else if (this.chainable){
            output.returnSymbol = "↩︎";
        } else if (this.augments){
            output.returnSymbol = "⇐";
            output.returnTypes = [this.augments[0]];
        }
    }
